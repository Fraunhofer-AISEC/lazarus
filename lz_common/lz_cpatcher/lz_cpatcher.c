...
void lz_core_patcher_run(void)
{
	...
	dbgprint(DBG_INFO2, "INFO: Applying Lazarus Core Update..\n");

	LZ_RESULT result = lz_apply_core_update();

	if (result == LZ_SUCCESS)
	{
		dbgprint(DBG_INFO2, "INFO: Successfully updated Lazarus Core. Rebooting..\n");
	}
	else
	{
...
static LZ_RESULT lz_apply_core_update(void)
{
	...
	if ((result = lz_get_staging_hdr(LZ_CORE_UPDATE, &hdr, (uint8_t*) lz_img_boot_params.info.cur_nonce))
		!= LZ_SUCCESS)
	{
...
static LZ_RESULT lz_flash_core_update(lz_auth_hdr_t *staging_elem_hdr)
{
	...
	dbgprint(DBG_INFO2,
		"Flashing staged update from staging area (0x%x) to update area (0x%x)\n",
