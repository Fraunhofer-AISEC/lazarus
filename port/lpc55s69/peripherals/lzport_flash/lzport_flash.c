...
bool lzport_flash_init(void)
{
	...
	VERB("Initializing flash driver...\n");
...
void lzport_flash_get_properties(void)
{
	if (0 == g_flash_config.PFlashTotalSize) {
		ERROR("Flash is not properly initialized\n");
		return;
	}
	...
	VERB("kFLASH_PropertyPflashBlockBaseAddr = 0x%X\n", pflashBlockBase);
	VERB("kFLASH_PropertyPflashSectorSize = %d\n", pflashSectorSize);
	VERB("kFLASH_PropertyPflashTotalSize = %d\n", pflashTotalSize);
	VERB("kFLASH_PropertyPflashPageSize = 0x%X\n", PflashPageSize);
}
...
bool lzport_flash_write(uint32_t start, uint8_t *buf, uint32_t size)
{
	...
	VERB("Flashing %d bytes from address 0x%X to address 0x%X\n", size, buf, flash_start);
...
bool lzport_flash_program_page(uint32_t start, uint8_t *buf)
{
	...
	if (!((flash_start < (FLASH_BASE_ADDR + FLASH_SIZE)) && (flash_start % FLASH_PAGE_SIZE) == 0)) {
		ERROR("ERROR: Failed to flash page. Address 0x%x outside of flash memory range"
			  "\n",
			  start);
		goto Cleanup;
	}
...
bool lzport_flash_erase_page(uint32_t start)
{
	...
	if (!((start < (FLASH_BASE_ADDR + FLASH_SIZE)) && (start % FLASH_PAGE_SIZE) == 0)) {
		ERROR("ERROR: Failed to erase page: address 0x%x outside of flash memory "
...
{
	VERB("Erasing flash from %d, size %d\n", start, size);
...
bool lzport_flash_read(uint32_t addr, uint8_t *buffer, uint32_t size)
{
	...
	VERB("FLASH - Reading from flash\n");

	if (flash_addr >= (FLASH_BASE_ADDR + FLASH_SIZE)) {
		ERROR("ERROR: Failed to read flash: address range 0x%x-0x%x is outside of "
			  "flash memory region\n",
			  addr, addr + size);
		return false;
	}
	...
	VERB("FLASH - Successfully read %d bytes from location 0x%X\n", size, addr);
	return true;
}
...
int lzport_retrieve_uuid(uint8_t uuid[LEN_UUID_V4_BIN])
{
	if (FFR_Init(&g_flash_config) != kStatus_Success) {
		ERROR("Failed to init FFR\n");
		return LZ_ERROR;
	}

	if (FFR_GetUUID(&g_flash_config, uuid) != kStatus_Success) {
		ERROR("Failed to retrieve UUID from protected flash\n");
		return LZ_ERROR;
	}
...
void verify_status(status_t status)
{
	...
	case kStatus_InvalidArgument:
		ERROR("Flash status: Invalid argument\n");
		break;
	case kStatus_FLASH_AddressError:
		ERROR("Flash status: Address error\n");
		break;
	case kStatus_FLASH_AlignmentError:
		ERROR("Flash status: Alignment Error\n");
		break;
	case kStatus_FLASH_AccessError:
		ERROR("Flash status: Access Error\n");
		break;
	case kStatus_FLASH_CommandNotSupported:
		ERROR("Flash status: This API is not supported in current target\n");
		break;
	case kStatus_FLASH_EccError:
		ERROR("Flash status: ECC Error\n");
		break;
	default:
		ERROR("Flash status: Other Error: 0x%X / %d\n", status, status);
		break;
	}
}
