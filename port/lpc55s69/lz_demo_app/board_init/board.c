...
void BOARD_InitDebugConsole_Core1(void)
{
	...
	DbgConsole_Init(BOARD_DEBUG_UART_INSTANCE_CORE1, BOARD_DEBUG_UART_BAUDRATE_CORE1,
					BOARD_DEBUG_UART_TYPE_CORE1, uartClkSrcFreq);
}
...
status_t BOARD_I2C_Send(I2C_Type *base, uint8_t deviceAddress, uint32_t subAddress,
...
status_t BOARD_I2C_Receive(I2C_Type *base, uint8_t deviceAddress, uint32_t subAddress,
...
{
	return BOARD_I2C_Receive(BOARD_ACCEL_I2C_BASEADDR, deviceAddress, subAddress, subaddressSize,
							 rxBuff, rxBuffSize);
}
...
status_t BOARD_Codec_I2C_Receive(uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize,
								 uint8_t *rxBuff, uint8_t rxBuffSize)
{
	return BOARD_I2C_Receive(BOARD_CODEC_I2C_BASEADDR, deviceAddress, subAddress, subAddressSize,
							 rxBuff, rxBuffSize);
}
#endif /* SDK_I2C_BASED_COMPONENT_USED */
